...
public:
    ...
    virtual double Evaluate(const Sheet& sheet) const = 0;
...
public:
    ...
    double Evaluate(const Sheet& sheet) const override {
        double result = 0;
...
static std::string valid_chars = "0123456789.";
...
public:
    ...
    double Evaluate(const Sheet& sheet) const override {
        ...
        if (!sheet.IsCellFulled(*cell_)) {
            return 0;
        }

        auto cell_ptr = sheet.GetConcreteCell(*cell_);

        if (cell_ptr->GetType() == CellType::EmptyType) {
            return 0;
        }
...
double FormulaAST::Execute(const Sheet& sheet) const {
